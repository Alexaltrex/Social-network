...
Оформляется списком или карточками. Каждый элемент содержит аватар пользователя, имя-ссылку на профиль пользователя, кнопку "добавить в друзья/удалить из друзей". Во время запроса дружбы кнопки становятся неактивными, показывается прелоадер:
